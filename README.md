...
> 运行在 Windows 本地的 AI 工程任务编排框架 —— Node.js CLI 统一入口 + Python FastAPI 控制面 + SQLite 持久化，将 Codex / Claude 等本机 Agent 与钉钉消息、任务队列、执行链路串联为完整的自动化工作流。
...
AI-Assistant 是一个本地优先（local-first）的 AI 工程任务编排框架。它把「提出需求 → 生成方案 → 澄清或自动执行 → Agent 执行 → 验证留痕」这条链路固化成可管理的服务，适合个人开发者或小团队在本机安全地调度 AI 编程助手完成真实工程任务。
...
- 🔄 **完整任务生命周期**：创建 → 规划 → 澄清或自动执行 → 验证 → 终态，支持暂停 / 恢复 / 取消 / 重试；钉钉任务需求明确时自动进入执行，CLI/API 任务保留人工审批；
- 🤖 **多 Agent 适配**：内置 Codex、Claude、zcode、DeepSeek、Mock 适配器，仅在显式启用且可检测到可执行文件时生效，不会静默调用未知命令；
- 💬 **钉钉双向集成**：消息意图识别、自动开始执行与澄清通知、富文本（text / richText）、高风险验收短口令（通过 / 驳回等）、任务进度推送；等待澄清时用户直接回复的内容会强制绑定当前任务，不会被误判为新任务或闲聊；
...
- 📋 **手动会话监管**：不抢占手工打开的 Codex / Claude 窗口，通过官方会话日志监管；多窗口并发时发现即运行的任务会补发开始通知，监控状态原子写入并带备份，不会因文件锁导致漏报；Codex 修订用原生 queue 插回原会话，Claude 用 resume 恢复；
...
- **自然语言操作**：创建任务、查询状态、取消任务、普通聊天；固定高置信指令（如 `运行`、`任务执行的怎么样了`、`TASK-xxx 结束`）直接走本地规则，无需云端模型；任务需求明确时规划完成即自动执行，信息不足时推送澄清问题；
- **审批短口令**：`通过 / 批准 / 同意 / 驳回` 作用于当前最新待审批任务（钉钉任务已免计划审批，主要用于高风险验收命令）；
- **人工卡点出口**：任务因验收修复耗尽或高风险命令等待人工时，`状态 <任务ID>` / `运行` 会展示真实卡点与失败验收命令；`重试 <任务ID>` 重新规划并执行（并入全部补充要求、重置修复次数），`重新验收 <任务ID>` 只重跑验收命令；
- **执行中补充 / 纠错与澄清绑定**：存在非终态任务时直接发送补充说明，或显式指定任务；任务等待澄清时，直接回复的任何实质内容都会并入当前任务的需求上下文并触发重新规划，不会丢失关联：
